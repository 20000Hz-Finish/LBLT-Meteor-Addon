...
import com.example.addon.mixin.IFriend;
...
public class FriendsManager extends Module {
    ...
    public FriendsManager() {
        super(com.example.addon.LBLTAddon.CATEGORY, "FriendsManager", "好友管理器");
        INSTANCE = this;
    }
    ...
    public final Setting<String> careFreePrefix = sgGeneral.add(new StringSetting.Builder()
        .name("CareFreePrefix")
        .description("CareFree前缀")
        .defaultValue(";")
        .build()
    );
    ...
    private void executeCommand(String command, Friend friend) {
        ...
        if (careFree.get()) {
            send(careFreePrefix.get() + command + " " + name);
        }
    }
